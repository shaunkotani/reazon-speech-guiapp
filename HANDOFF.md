...
- 元音声の全体プレビュー。
- 取込後は「音声に合う設定 → 仕上がりをテスト → 全体を文字起こし」の3工程を常時表示する。インタビュー／会議／電話・通話／講演・一人語りを主選択とし、迷った場合の「通常」と保存済みプリセットを使う「カスタム」も選べる。いずれかの明示選択は必須で、「カスタム」は保存済み設定を選ぶまで後工程を開始できない。
- 「仕上がりをテスト」から独立モーダルを開き、開始位置と30秒/60秒/120秒を指定して同じパイプラインで試し文字起こしできる。既定は冒頭60秒。指定範囲だけの音声プレイヤーも表示し、範囲変更時に再生成する。確認後は全体実行を主操作に切り替える。

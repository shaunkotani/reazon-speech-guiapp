...
3. 先頭ワーカーが FFmpeg デコード、ノイズ除去、Silero VAD を行い、16 kHz mono float32 PCM を一時ファイルへ保存する。重なり解析が有効かつノイズ除去を使う場合は原音PCMも別に保存する。
4. 重なり解析のキャッシュがなければ、main が原音を120秒 core＋前後5秒の文脈へ分けて最大4ワーカーへ並列分配する。
5. `shared/overlap.js` が VAD 区間を最大長以内へ分割し、重なり補正が有効なら再認識候補を追加する。
6. main が通常区間と最大3件の初回補正候補を全ワーカーへ配り、合意できない補正区間だけ残りの候補を追加認識する。
7. 重なり区間は複数候補の編集距離による合意度で採用結果を決め、通常区間と時刻順に統合する。
8. 話者タグ付けをユーザーが開始したときだけ、同じ区間列の WeSpeaker 埋め込みを遅延計算する。
...
- pyannote + WeSpeaker の diarizer は重なり解析を担当した各ワーカーで遅延生成し、後続ジョブでも再利用する。
...
- 通常実行の話者解析は120秒ごと（前後5秒の文脈付き）に分割し、最大4ワーカーへ並列分配する。各ワーカーのONNXスレッド数は論理CPU数から合計予算を求め、1〜4で自動調整する。採用範囲を重ならない120秒の core へ切るため、前後文脈の解析結果は二重計上しない。
- 原音・試行範囲・想定話者数・話者モデルが同じ重なり解析結果は、mainプロセス内の最大12件LRUへ保持する。VAD・ノイズ除去だけを変更した再試行では重なり解析を再実行しない。
- 再認識は各話者境界について、広い・中間・狭い最大3候補を先に処理する。2票以上かつ合意度0.55以上なら残りを省略し、候補が割れた区間だけ最大9候補へ拡張する。
...
- pyannote diarizer は重なり解析チャンクを割り当てられたワーカーにだけ遅延生成する。
...
- `test:overlap-worker`: 本番 asrWorker でチャンク話者解析と段階的な補正候補認識を通し、標準2話者サンプルの最終文が全候補認識と一致することを確認する。
